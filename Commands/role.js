...
module.exports = {
    ...
    execute(client, message, args, func){
        try{
            ...
            try{
                func.jsonReader('./database.json', (err, database) => {
                    if(err){ hibaUzenetDelete("Error: Role - Reading file"); console.log("Error: Role - Reading file:", err); return; }
                    ...
                    if(!RoleExists) { hibaUzenetDelete("Nincs ilyen felhasználó regisztrálva!"); return; }
        ...
        }catch(error){ hibaUzenetDelete("Error: role command"); console.log("Error: role command", error); return; }
