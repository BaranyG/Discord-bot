...
module.exports = {
    ...
    execute(Discord, client, func, message, args){
        try{
            if(args[0]===undefined){ 
                message.channel.send("'!birthday [születési dátum]' vagy '!birthday format' "); 
                return; 
            }
                ...
                +"**__Szabályok:__** 1920-2010-ig lehet évet megadni. Szökőnapot csak szökőévvel, vagy év nélkül lehet. (Ez esetben a nem szökőévekben 28-án fog értesíteni.)"
                +"Nem lehet utána írni semmit. Nem lehet rövidebb, vagy hosszabb, mint egy érvényes dátum."
                +"Nem lehet benne más, számon és ponton kívül, azokból is ellenőrízve van, hol helyezkednek el. Aki rossz szándékkal megpróbálja kifagyasztani azt kizárom.");
                return; 
            }
            ...
            else if(args[0].length < 3 || args[0].length > 11){ 
                hibaUzenetDelete("Error: Nem megfelelő hosszúságú dátum."); 
                return; 
            }
            else if(!(args[0].includes('.')) || !(func.hasNumber(args[0]))){ 
                hibaUzenetDelete("Error: Hiányzik 'pont' vagy szám -> !birthday formátum"); 
                return; 
            }
            else if(!func.formatDate(args[0])) { 
                hibaUzenetDelete("Error: Nem megfelelő formátum: [!birthday formátum]"
                +"\nVagy nem érvényes dátumot adtál meg."); 
                return; 
            }
            else if(!func.validDate(args[0])) { 
            ...
            try{
                ...
                func.jsonReader('./database.json', (err, database) => {
                    ...
                    for(let i = 0; i < database.Tagok.length; i++){
                        if(database.Tagok[i].UserID === Tag.UserID && database.Tagok[i].ServerID === Tag.ServerID) { 
                            hibaUzenetDelete("Te már szerepelsz az adatbázisban!"); 
                            return; 
                        }
                    }
                ...
                hibaUzenetDelete("Error #B1"); 
                console.log("Error #B1", error); 
                return; 
            }
            return;
        } catch(e){
            hibaUzenetDelete("Error #B0");
            console.log("Error: #B0", e);
        }
